...
export function validateStoreUser(body: Record<string, any>): StoreUserData {
  ...
  if (!username || typeof username !== 'string') {
    ...
    errors.push({ sourceField: 'username', rule: 'between', detail: 'The username must be between 1 and 255 characters.' });
  ...
  if (!firstName || typeof firstName !== 'string') {
    ...
    errors.push({ sourceField: 'first_name', rule: 'between', detail: 'The first name must be between 1 and 255 characters.' });
  }
  ...
  if (!lastName || typeof lastName !== 'string') {
    ...
    errors.push({ sourceField: 'last_name', rule: 'between', detail: 'The last name must be between 1 and 255 characters.' });
  }

  if (password !== undefined && password !== null && password !== '') {
    if (typeof password !== 'string' || password.length < 8) {
      errors.push({ sourceField: 'password', rule: 'min', detail: 'The password must be at least 8 characters.' });
    }
  }

  if (language !== undefined && language !== null) {
    if (typeof language !== 'string' || language.length < 2 || language.length > 5) {
      errors.push({ sourceField: 'language', rule: 'between', detail: 'The language must be between 2 and 5 characters.' });
    }
  }

  if (externalId !== undefined && externalId !== null) {
    if (typeof externalId !== 'string' || externalId.length > 255) {
      errors.push({ sourceField: 'external_id', rule: 'max', detail: 'The external id must not exceed 255 characters.' });
    }
  }
