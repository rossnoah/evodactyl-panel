...
const sessionStore = config.session.driver === 'database' ? new MysqlSessionStore(config.session.lifetime) : undefined;
...
function injectBootstrap(html: string, bootstrap: Bootstrap): string {
    ...
    if (/name="csrf-token"/.test(out)) {
        out = out.replace(/<meta\s+name="csrf-token"\s+content="[^"]*"\s*\/?\s*>/, csrfMeta);
    } else {
