...
export const rename = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const description = req.body.description !== undefined ? String(req.body.description) : server.description;
...
export const reinstall = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        await activityFromRequest(req).event('server:reinstall').subject(server, 'Pterodactyl\\Models\\Server').log();
