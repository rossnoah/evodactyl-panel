...
export class BackupController {
    ...
    async index(req: Request, res: Response): Promise<void> {
        ...
        const perPage = Math.min(parseInt(req.query.per_page as string, 10) || 20, 500);
    ...
    async store(req: Request, res: Response): Promise<void> {
        ...
        const response = await fractal(req).item(backup).transformWith(transformer).toArray();
    ...
    async download(req: Request, res: Response): Promise<void> {
        ...
        const url = await downloadService.handle({ ...backup, server: { ...server, node: server.nodes } }, user);
    ...
    async restore(req: Request, res: Response): Promise<void> {
        ...
        if (backup.disk === 's3') {
            const downloadService = new DownloadLinkService();
            url = await downloadService.handle({ ...backup, server: { ...server, node: server.nodes } }, user);
        }
