...
export const getDeployableNodes = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const memory = parseInt(req.query.memory as string, 10) || 0;
        const disk = parseInt(req.query.disk as string, 10) || 0;

        // Parse location_ids from query
        let locationIds: number[] = [];
        const rawLocationIds = req.query.location_ids;
        if (Array.isArray(rawLocationIds)) {
            locationIds = rawLocationIds.map((id) => parseInt(id as string, 10)).filter((id) => !Number.isNaN(id));
