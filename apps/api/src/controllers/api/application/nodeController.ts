...
import { normalizeNodeData, validateStoreNode } from '../../../validation/schemas/node.js';
...
export const view = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const transformer = NodeTransformer.fromRequest(req);
        const response = await fractal(req).item(node).transformWith(transformer).toArray();
