...
export const transfer = async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        // Generate a JWT for the destination node. Matches Laravel's
        // ServerTransferController: 15 min expiry, sha256 identifier, subject = server uuid.
        const token = new NodeJWTService()
            .setExpiresAt(new Date(Date.now() + 15 * 60 * 1000))
            .setSubject(server.uuid)
            .handle(targetNode, server.uuid, 'sha256');
