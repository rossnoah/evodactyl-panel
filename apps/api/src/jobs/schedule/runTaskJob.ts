    ...
    try {
        switch (fullTask.action) {
            case 'power': {
                ...
                await (repo as any).post(`/api/servers/${server.uuid}/power`, { action: fullTask.payload });
                break;
            }
            ...
            case 'command': {
                const repo = new DaemonRepository();
                repo.setNode(server.nodes);
                await (repo as any).post(`/api/servers/${server.uuid}/commands`, { commands: [fullTask.payload] });
                break;
            }

            case 'backup': {
                const backupService = new InitiateBackupService();
                const ignoredFiles = fullTask.payload ? fullTask.payload.split('\n') : [];
