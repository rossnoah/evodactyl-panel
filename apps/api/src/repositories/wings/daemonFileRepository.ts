...
export class DaemonFileRepository extends DaemonRepository {
    ...
    async getContent(path: string, notLargerThan?: number): Promise<string> {
        const response = await this.get(`/api/servers/${this.server.uuid}/files/contents`, { file: path });
    ...
    async putContent(path: string, content: string): Promise<Response> {
        ...
        if (!response.ok) {
            throw new DaemonConnectionException(`Failed to write file: ${response.status} ${response.statusText}`);
        }
    ...
    async getDirectory(path: string): Promise<any[]> {
        const response = await this.get(`/api/servers/${this.server.uuid}/files/list-directory`, { directory: path });
    ...
    async compressFiles(root: string | null, files: string[]): Promise<any> {
        ...
        if (!response.ok) {
            throw new DaemonConnectionException(`Failed to compress files: ${response.status} ${response.statusText}`);
        }
