...
import { ask, askSecret, confirm, createPromptInterface, parseOptions } from '../prompt.js';
...
export async function makeUserCommand(args: string[]): Promise<number> {
    ...
    try {
        ...
        const nameFirst = options.get('name-first') ?? (await ask(rl, 'First name'));
        ...
        if (password === undefined && !options.has('no-password')) {
            console.warn('Password must be at least 8 characters long and contain upper, lower, and a number.');
