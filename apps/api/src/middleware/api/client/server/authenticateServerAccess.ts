...
export async function authenticateServerAccess(req: Request, _res: Response, next: NextFunction): Promise<void> {
    ...
    const server = await prisma.servers.findFirst({
        where: {
            OR: [{ uuid: serverIdentifier }, { uuidShort: serverIdentifier }],
        },
        include: {
            allocations: {
    ...
    if (server.status === 'suspended' && !isAdmin) {
        return next(
            new ServerStateConflictException(
    ...
    if (
        ...
        return next(new ServerStateConflictException('This server has not yet completed its installation process.'));
    }
